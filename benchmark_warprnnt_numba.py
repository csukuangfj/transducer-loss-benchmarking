...
def compute_loss(loss_func, logits, logit_lengths, targets, target_lengths):
    with record_function("warprnnt_numba"):
        loss = loss_func(
            acts=logits,
            labels=targets,
...
def main():
    ...
    loss_func = warprnnt_numba.RNNTLossNumba(
        blank=0, reduction="sum", fastemit_lambda=0.0, clamp=0.0
    )
    ...
    with open(f"warprnnt_numba-{suffix}.txt", "w") as f:
        f.write(s + "\n")
